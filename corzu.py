...
import copy, operator, re, sys, pdb, os, subprocess
...
#def main(file1,file2='',file3=''):
    ...
    if mode=='train':
        if not os.path.isfile(file1):
            sys.stderr.write(file1,'does not exist')
            return
    else:
        if not os.path.isfile(file1):
            sys.stderr.write(file1,'does not exist')
        ...
        if not os.path.isfile(file2):
            sys.stderr.write(file2,'does not exist')
            return
    ...
    if mode=='train':
        assert("NOT HERE")
        sys.stderr.write('Training on file',file1)
    if mode=='test':
        pass
        #sys.stderr.write('Testing on file',file1)
                    ...
                    if matched==1:
                        try:
                            ...
                            if mable_cset!=ante_cset:
                                sys.stderr.write('\n',docid)
                                sys.stderr.write('\nWrong resolution')
                                sys.stderr.write(mable,ante)
                                #pdb.set_trace()
                        except StopIteration:
                            pass
                            sys.stderr.write('\n',docid)
                            sys.stderr.write('\nResolved a non gold mention')
                            sys.stderr.write('ante:',ante,'\nmable:',mable)
                            #pdb.set_trace()
                    else:
                        try:
                            ...
                            if not coref[mable_cset].index(mable[1:4])==0:
                                ...
                                sys.stderr.write(mable)
                                sys.stderr.write('cset')
                                for tm in coref[mable_cset]:
                                    sys.stderr.write(get_mable(tm))
                                if doc_counter>50:
                                    pdb.set_trace()
                        except StopIteration:
                            pass
            ...
            if not mable[4] in ['NN','NE']:# and len(all_antes)>1:
                ...
                """
                    ...
                    try:
                        ...
                        sys.stderr.write(ante_cands)
                        sys.stderr.write(ante_cands_csets)
    ...
    if mode=='test':
        ...
        for line in parzulines:   #open(file2,'r').readlines():
            if len(line) == 0 or line=='\n':
                ...
                if csets_out==[] and mables_out==[]:
                    outlines.append('\t'.join(line[0:-1])+'\t-')
                elif csets_out==[] and mables_out!=[]:
                    outlines.append('\t'.join(line[0:-1])+'\t'+'|'.join(mables_out))
                elif csets_out!=[] and mables_out==[]:
                    outlines.append('\t'.join(line[0:-1])+'\t'+'|'.join(csets_out))
                else:
                    outlines.append('\t'.join(line[0:-1])+'\t'+'|'.join(csets_out)+'|'+'|'.join(mables_out))
    ...
    if mode=='test' and output_pronoun_eval:
                ...
                for lemma,c in eval_all[pos].items():
                    ...
                    if pos in ['PPER','PPOSAT'] and lemma in ['er','sie','sein','ihr']:
                        ...
                        if not c['true_ante_present']==0:
                            sys.stderr.write("%.2f" % (100*float(c['tp'])/c['true_ante_present']),'% ('+str(c['tp'])+')\t\t',)
                            sys.stderr.write('true ante present in '+str(c['true_ante_present'])+' of '+str(c['instances'])+' cases ('+"%.2f" % (100*float(c['true_ante_present'])/c['instances'])+'%),',)
                            if not pos_true_mentions==0:
                                sys.stderr.write(c['true_mention'],'true mentions ('+"%.2f" % (100*float(pos_true_ante_present)/pos_true_mentions)+'%)')
                            else:
                                sys.stderr.write(c['true_mention'],'true mentions')
                        else:
                            sys.stderr.write('\t\tno true ante in '+str(c['instances'])+' cases,',c['true_mention'],'true mentions')
                acc=(100*float(pos_tp)/pos_true_ante_present) if not pos_true_ante_present==0 else 0.
                accuracies[pos]=acc
                sys.stderr.write('ALL:\t',"%.2f" % (acc),'% ('+str(pos_tp)+')\t\t',)
                #sys.stderr.write('ALL:\t',"%.2f" % (100*float(pos_tp)/pos_true_ante_present),'% ('+str(pos_tp)+')\t\t',)
                sys.stderr.write('true ante present in '+str(pos_true_ante_present)+' of '+str(pos_cases)+' cases ('+"%.2f" % (100*float(pos_true_ante_present)/pos_cases)+'%),',)
                acc2=100*float(pos_true_ante_present)/pos_true_mentions if not pos_true_mentions==0 else 0
                sys.stderr.write(pos_true_mentions,'true mentions ('+"%.2f" % (acc2)+'%)')
                all_tp+=pos_tp
        ...
        sys.stderr.write('OVERALL:\t',"%.2f" % (100*float(all_tp)/all_true_ante_present),'% ('+str(all_tp)+')\t\t',)
        sys.stderr.write('true ante present in '+str(all_true_ante_present)+' of '+str(all_cases)+' cases ('+"%.2f" % (100*float(all_true_ante_present)/all_cases)+'%),',)
        sys.stderr.write(all_true_mentions,'true mentions ('+"%.2f" % (100*float(all_true_ante_present)/all_true_mentions)+'%)')
        ...
        for pos in eval_ante_index:
            ...
            for lexem in eval_ante_index[pos]:
                sys.stderr.write(lexem)
                for r,c in eval_ante_index[pos][lexem].items():
                    sys.stderr.write(str(r+1)+':',c,)
                sys.stderr.write('')
            sys.stderr.write('')
        """
    ...
    if verb_postfilter=='train':
        ...
        f.close()
        sys.stderr.write('verb_postfilter_cnt:',verb_postfilter_cnt)
    if verb_postfilter=='test':
        tmp=wapiti_verb_postfilter.communicate()    #close it
        sys.stderr.write('verb_postfilter_cnt:',verb_postfilter_cnt)

    if link_ne_nn_sets:
        sys.stderr.write('eval_link_ne_nn_sets',dict(eval_link_ne_nn_sets))
    ...
    if verb_postfilter_context_w2v or verb_postfilter_context_graph:
        ...
        for pos,lemmas in accuracy.items():
            ...
            for lemma,counts in lemmas.items():
                sys.stderr.write('\t',lemma,'\t',)
                for k in sorted(counts): sys.stderr.write(k+':',counts[k],'\t',)
                sys.stderr.write('total:',sum(counts.values()))
                sys.stderr.write('\t\tbaseline acc:',"%.2f" % (100.*float(counts['baseline correct']+counts['both correct']) / sum(counts.values())),'%',)
                sys.stderr.write('verb_sel acc:',"%.2f" % (100.*float(counts['verb_sel correct']+counts['both correct']) / sum(counts.values())),'%',)
                sys.stderr.write('upper bound:',"%.2f" % (100.*float(counts['baseline correct']+counts['both correct']+counts['verb_sel correct']) / sum(counts.values())),'%')
    return "\n".join(outlines) + "\n"
